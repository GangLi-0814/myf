...
program define myf
    version 15.1
    syntax anything(name = title)[,Logo(string) Url(string) Saving(string) Path(string)]
	...
	* Week --> 背景颜色
	...
	local wday = dow(date("`c(current_date)'"),"DMY") // 周几
	if `wday' == 0{
		local bc "blue*3.8"
